	...
	<div class="rtcl-settings-nav-wrap">
		<ul class="nav-tab-wrapper">
			...
			foreach ( $this->tabs as $slug => $title ) {
				...
				echo '<li>';
				echo '<a href="?post_type=' . esc_attr( rtcl()->post_type ) . '&page=rtcl-settings&tab=' . esc_attr( $slug ) . '" class="' . esc_attr( $class ) . '">' . esc_html( $title ) . '</a>';
		...
		if ( ! empty( $this->subtabs ) ) {
			...
			foreach ( $this->subtabs as $id => $label ) {
				echo '<li><a href="' . esc_url( admin_url( 'edit.php?post_type=' . rtcl()->post_type . '&page=rtcl-settings&tab=' . $this->active_tab
														   . '&section=' . sanitize_title( $id ) ) ) . '" class="nav-sub-' . esc_attr( strtolower( $label ) ) . ( $this->current_section == $id ? ' current' : '' ) . '">'
					 . esc_html( $label )
					 . '</a></li>';
			}
			echo '</ul>';

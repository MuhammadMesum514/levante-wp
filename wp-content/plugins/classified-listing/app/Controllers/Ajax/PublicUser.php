...
class PublicUser {
	...
	public static function profile_picture_delete() {
		$message = null;
		if ( wp_verify_nonce( isset( $_REQUEST[ rtcl()->nonceId ] ) ? $_REQUEST[ rtcl()->nonceId ] : null, rtcl()->nonceText ) && $user_id = get_current_user_id() ) {
	...
	public static function profile_picture_upload() {
		$msg = null;
		if ( wp_verify_nonce( isset( $_REQUEST[ rtcl()->nonceId ] ) ? $_REQUEST[ rtcl()->nonceId ] : null, rtcl()->nonceText ) && isset( $_FILES['pp'] ) && $user_id = get_current_user_id() ) {
	...
	public function rtcl_update_user_account() {
		...
		if ( ! apply_filters( 'rtcl_listing_form_remove_nonce', false ) && ! wp_verify_nonce( isset( $_REQUEST[ rtcl()->nonceId ] ) ? $_REQUEST[ rtcl()->nonceId ] : null, rtcl()->nonceText ) ) {
					...
					if ( ! Functions::notice_count( 'error' ) ) {
						...
						if ( ! is_user_logged_in() && $post_for_unregister ) {
							...
							if ( $new_user_id && is_numeric( $new_user_id ) ) {
								...
								Functions::add_notice( apply_filters( 'rtcl_listing_new_registration_success_message', sprintf( esc_html__( 'A new account is registered, password is sent to your email(%s).', 'classified-listing' ), $meta['email'] ), $meta['email'] ) );
							}
						}
						if ( $user_id ) {

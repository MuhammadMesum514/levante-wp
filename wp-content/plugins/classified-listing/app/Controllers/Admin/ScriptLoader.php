...
class ScriptLoader {
	...
	function localization_both_end() {
		...
		if ( Functions::is_listing_form_page() || ( is_admin() && in_array(
			$pagenow,
			[
				'post.php',
				'post-new.php',
			...
			$fromBuilderParams = apply_filters(
				'rtcl_localize_params_form_builder',
				[
					'isAdminEnd' => is_admin(),
					'hasPro'     => rtcl()->has_pro(),
					'forms'      => $forms,
	...
	function register_script() {
		...
		if ( $recaptcha_version == 3 ) {
			wp_register_script( 'rtcl-recaptcha', 'https://www.google.com/recaptcha/api.js?render=' . esc_attr( $misc_settings['recaptcha_site_key'] ), '', RTCL_VERSION );
		} else {
			wp_register_script( 'rtcl-recaptcha', 'https://www.google.com/recaptcha/api.js?onload=rtcl_on_recaptcha_load&render=explicit', '', $this->version, true );
		}
		...
		if ( is_singular( rtcl()->post_type ) ) {
			...
			$message                = sprintf( esc_html__( "Need to discuss something related to '%1\$s' from %2\$s", 'classified-listing' ), $post->post_title, get_permalink( $post->ID ) );
	...
	function register_admin_script() {
		...
		wp_register_style( 'jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/themes/smoothness/jquery-ui.css', '', '1.12.1' );
	...
	public function get_map_localized_options() {
		...
		$center_point  = ! empty( $center_point ) && is_array( $center_point ) ? wp_parse_args(
			$center_point,
			[
				'address' => '',
				'lat'     => 0,

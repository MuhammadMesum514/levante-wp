...
class AdminSettings extends SettingsAPI {
	...
	function setTabs() {
		...
		// Find the active tab
		$this->option = $this->active_tab = ! empty( $_GET['tab'] ) && array_key_exists( $_GET['tab'], $this->tabs ) ? trim( $_GET['tab'] ) : 'general'; /* phpcs:ignore WordPress.Security.NonceVerification.Recommended */
		$this->add_subsections();

		if ( ! empty( $this->subtabs ) ) {
			$this->current_section = ! empty( $_GET['section'] ) && array_key_exists( $_GET['section'], $this->subtabs ) ? trim( $_GET['section'] ) : ''; /* phpcs:ignore WordPress.Security.NonceVerification.Recommended */
